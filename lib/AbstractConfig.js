...
class AbstractConfig {
     ...
     * 
     * @param {Object} option 
     * @param {Number} [option.waitTimeBeforeThrowErrorMs=0] The wait time before trigger error when the value returned is not expected. The default value is `0`, but it will throw the error in async way even if you set the parameter to `0`.
     */
    constructor(option) {
     ...
     */
    loadVar(key) {
        slogger.trace('begin to load the key', key);
        if (!this._usePromise) {
    ...
    _checkNecessaryObject(key, obj) {
        if (typeof (obj) === 'string') {
            try {
                ...
                slogger.error('parse error',e, obj);
        ...
        return this._exitProcess('the value of ' +key+' is a necessary object ,but get type '+ typeof (obj) + ' ' + obj);
    }
    /**
    ...
    loadNecessaryObject(key) {
        if (!this._usePromise) {
            return this._checkNecessaryObject(key, this.loadNecessaryVar(key));
        }
        const _this = this;
        const promise = this.loadNecessaryVar(key).then(function(obj) {
    ...
    loadNecessaryFile(key,onlyCheckDirectory) {
        if (!this._usePromise) {
            return this._checkNecessaryFile(key, this.loadNecessaryString(key), onlyCheckDirectory);
        }
        const _this = this;
        return this.loadNecessaryString(key).then(function(filePath) {
