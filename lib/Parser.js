 ...
 * 
 * @property {*|Function} [preferred] when it not empty , it will been used instead of the value get from consul or json object.
 * It can be an function or a value of type of String/Number/Object etc.
...
class Parser {
    ...
    _validateItem({schemaElement, key, value}) {
        if (typeof (schemaElement.preferred) === 'function') {
            const preferredValue = schemaElement.preferred();
            if (!this._isEmptyValue(preferredValue)) {
                value = preferredValue;
            }
        } else if (!this._isEmptyValue(schemaElement.preferred)) {
            value = schemaElement.preferred;
        }
    ...
    doParseElement({key, value, isFromWatch=false, oldValue}) {
        ...
        if (typeof (schemaElement.afterParse) === 'function') {
            if (!isFromWatch || changed) {
                schemaElement.afterParse(returnValue, isFromWatch);
            }
        }
